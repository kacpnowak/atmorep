...
class DataLoader:
    ...
    def get_field(self, year, month, field, level_type, vl,
        ...
        t_srate = t_pad[2]
        # Assume that latitude and longitude are stored as two last values of the file shape
        data_ym = torch.zeros((0, self.file_shape[-2], self.file_shape[-1]))

        # pre-fill fixed values
        if 'fesom' == self.data_type:
            fname = self.fname_base.format(self.path, field, field, self.data_type, year)
            print(fname)
            x = self.file_loader(fname, self.grib_index[field], [0, 0, t_srate], (year, month, vl))
    ...
    def get_static_field(self, field, token_size=[-1, -1]):
        if 'fesom' == self.data_type:
            raise NotImplemented("Static fields are not supported for FESOM data")
